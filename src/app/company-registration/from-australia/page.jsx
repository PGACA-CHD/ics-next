...
function CountUp({ end, suffix = '', prefix = '', delay = 0 }) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => { const id = setInterval(() => { i++; const p = 1 - Math.pow(1 - i / steps, 3); setVal(Math.round(end * p)); if (i >= steps) clearInterval(id); }, 1300 / steps); }, delay);
  }, [vis]);
...
const whyPoints = [
  ["Tap India's 1.4 billion consumer market", "The Australia-India trade relationship is one of the fastest growing in the Indo-Pacific — a direct India entity gives you a permanent foothold."],
  ["World-class tech and engineering talent", "Access one of the world's largest pools of software engineers, CAs, and technical professionals at a fraction of Australian cost."],
  ["ECTA trade agreement benefits", "The Australia-India Economic Cooperation and Trade Agreement (ECTA) provides significant tariff reductions and priority market access for Australian businesses."],
  ...
  ["Repatriate profits to Australia", "Dividends are freely repatriable under FEMA/RBI guidelines and the Australia-India Double Taxation Agreement."],
  ["100% FDI in most sectors", "Automatic Route FDI permitted in IT, consulting, manufacturing, e-commerce, and more — no government pre-approval needed."],
];
...
const faqs = [
  ...
  { q: 'How long does it take to register a company in India from Australia?', a: 'Typically 15–25 working days once all apostilled documents are ready. The DFAT apostille takes approximately 2–5 working days.' },
  { q: 'What does the Australia-India ECTA mean for my business?', a: 'The ECTA (2022) provides Australian businesses preferential tariff treatment on goods exported to India and simplifies market entry for services. Having an India entity positions you to maximise these benefits.' },
];
...
const docIcons = [
  ...
  <svg key="i5" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#c8870a" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><rect x="4.5" y="3" width="15" height="18" rx="1.5" /><line x1="8.5" y1="7.5" x2="10.5" y2="7.5" /><line x1="13.5" y1="7.5" x2="15.5" y2="7.5" /></svg>,
];
...
function DocRow({ doc, i, isMobile, reduced }) {
  ...
  const contentStyle = { opacity: show ? 1 : 0, transform: show ? 'translateX(0)' : `translateX(${slideFrom}px)`, transition: reduced ? 'none' : 'opacity .6s ease .15s, transform .6s cubic-bezier(0.22,1,0.36,1) .15s' };
  const nodeStyle = { opacity: show ? 1 : 0, transform: show ? 'scale(1)' : 'scale(0.4)', transition: reduced ? 'none' : 'opacity .45s ease, transform .55s cubic-bezier(0.34,1.56,0.64,1)' };
  ...
  return (
    <div ref={ref} className={`doc-tl-row ${left ? 'tl-left' : 'tl-right'}`} style={{ display: 'grid', gridTemplateColumns: '1fr 88px 1fr', alignItems: 'center', minHeight: 118 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', position: 'relative' }}>
        <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#fff', border: `1.5px solid ${show ? 'rgba(200,135,10,0.45)' : 'rgba(200,135,10,0.28)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: show ? '0 8px 22px rgba(200,135,10,0.16)' : 'none', position: 'relative', zIndex: 1, ...nodeStyle }}>
...
function DocTimeline({ items }) {
  ...
  useEffect(() => {
    ...
    let raf = 0;
    const update = () => { raf = 0; const el = wrapRef.current; if (!el) return; const rect = el.getBoundingClientRect(); const p = (window.innerHeight * 0.7 - rect.top) / rect.height; setProgress(Math.max(0, Math.min(1, p))); };
    ...
    <>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; }
        ...
        .why-anim-card {
          flex:1; border-radius:16px; border:1px solid rgba(9,48,36,0.18);
          padding:32px 28px; position:relative; overflow:hidden; cursor:default;
          background: linear-gradient(135deg, #e8f5ee 0%, #c8ead8 18%, #f0faf4 35%, #a8dfc0 52%, #ddf2e8 68%, #b8e8cc 82%, #e8f5ee 100%);
          background-size:400% 400%; animation:whyGrad 5s ease infinite;
        ...
        .why-anim-card:hover { box-shadow:0 14px 40px rgba(9,48,36,0.16); transform:translateY(-3px); border-color:rgba(9,48,36,0.32); }
        .why-anim-card::before { content:''; position:absolute; top:0; left:0; right:0; height:3px; background:linear-gradient(90deg,#093024 0%,#1a6b42 25%,#4caf82 50%,#1a6b42 75%,#093024 100%); background-size:300% 100%; animation:whyGrad 3s ease infinite; }
        ...
        <div style={{ maxWidth: 1160, margin: '0 auto' }}>
          <Fade>
            <div style={{ textAlign: 'center', marginBottom: 36 }}>
              ...
              <h2 style={{ fontSize: 'clamp(22px,3vw,36px)', fontWeight: 800, letterSpacing: '-0.025em', color: '#111', marginBottom: 8, fontFamily: HV }}>Why Australian Businesses Are Setting Up in India</h2>
          ...
          <Fade>
            <div style={{ textAlign: 'center', marginBottom: 56 }}>
              ...
              <p style={{ fontSize: 14.5, color: '#666', lineHeight: 1.75, fontFamily: HV, maxWidth: 560, margin: '0 auto' }}>
                Australian documents must be apostilled through DFAT. Both Australia and India are signatories to the Hague Apostille Convention.
              </p>
            </div>
          </Fade>

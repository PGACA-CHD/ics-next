...
/* ── RippleGrid — inlined directly in this file (React Bits, JS + CSS variant) ── */
function RippleGrid({
  enableRainbow = false,
  gridColor = '#ffffff',
...
/* one distinct accent color PER service — but each service only ever uses its own single color throughout */
const SVC_COLOR = {
  ...
  fema: { acc: "#a30010", bg: "rgba(215,0,21,0.09)", bdr: "rgba(215,0,21,0.30)" },
  transfer_pricing: { acc: "#1a6b42", bg: "rgba(26,107,66,0.09)", bdr: "rgba(26,107,66,0.30)" },
};
    ...
    <div style={{ fontFamily: HV, color: "#111", background: "#fff" }}>
      <style>{`
        *{box-sizing:border-box;}
        ...
        /* ── flip card: mobile-first (static, stacked, no gradient) ── */
        .flip-wrap{cursor:default;}
        .flip-inner{position:relative;display:flex;flex-direction:column;}
        .flip-front,.flip-back{position:relative;border-radius:16px;padding:24px 22px;border:${BDR};display:flex;flex-direction:column;}
        .flip-front{background:${GLASS};backdrop-filter:${BLUR};-webkit-backdrop-filter:${BLUR};border-radius:16px 16px 0 0;border-bottom:none;}
        .flip-back{background:${GREEN};border-radius:0 0 16px 16px;border-top:1px solid rgba(255,255,255,0.15);}
        .mob-hide{display:none;}

        /* ── desktop only: hover-to-flip + gradient ── */
        @media(min-width:961px){
          ...
          .flip-front,.flip-back{position:absolute;inset:0;backface-visibility:hidden;-webkit-backface-visibility:hidden;border-radius:16px;border-bottom:${BDR};}
          .flip-back{background:linear-gradient(135deg,#093024 0%,#1a5c3a 60%,#0d4028 100%);transform:rotateY(180deg);border-top:none;}
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 1 }}>
          <div className="hero-g" style={{ display: "grid", gridTemplateColumns: "1fr 460px", gap: 64, alignItems: "center" }}>
            <Fade>
              ...
              <h1 style={{ fontSize: "clamp(40px,5.5vw,68px)", fontWeight: 800, lineHeight: 1.04, letterSpacing: "-0.033em", color: "#111", margin: "0 0 22px", fontFamily: HV }}>
              ...
              <div className="stat-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
                {[
                  ...
                  <div key={i} style={{ background: GLASS, backdropFilter: BLUR, WebkitBackdropFilter: BLUR, border: BDR, borderRadius: 14, padding: "16px 12px", textAlign: "center" }}>
                    <div style={{ fontSize: "clamp(22px,2.5vw,28px)", fontWeight: 800, color: GREEN, lineHeight: 1, fontFamily: HV }}>
      ...
      <section className="sec" style={{ padding: "88px 56px", background: "#fff" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <Fade>
            <div className="lbl" style={{ marginBottom: 8 }}>Our Post Setup Services</div>
            <h2 style={{ fontSize: "clamp(24px,3vw,38px)", fontWeight: 800, letterSpacing: "-0.025em", color: "#111", margin: "0 0 8px", fontFamily: HV }}>Everything your India entity needs to stay compliant.</h2>
            <p style={{ fontSize: 15, color: "#666", margin: "0 0 28px", fontFamily: HV }}>Select each service to see exactly what's covered — monthly and annual.</p>
          ...
          <div className="svc-g" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            <div className="gc" style={{ padding: "28px 24px", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", inset: 0, backgroundImage: "radial-gradient(circle,rgba(9,48,36,0.035) 1px,transparent 1px)", backgroundSize: "18px 18px", pointerEvents: "none" }} />
            ...
            <div className="gc" style={{ padding: "28px 24px", display: "flex", flexDirection: "column" }}>
      ...
      {/* ── WHY RETAINER — flip (desktop, gradient) / static stacked (mobile, flat green) ── */}
      <section className="sec" style={{ padding: "88px 56px", background: "#fff", borderTop: "1px solid rgba(0,0,0,.07)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <Fade>
            <div className="lbl" style={{ marginBottom: 8 }}>Why a Retainer</div>
            <h2 style={{ fontSize: "clamp(24px,3vw,38px)", fontWeight: 800, letterSpacing: "-0.025em", color: "#111", margin: "0 0 44px", fontFamily: HV }}>Why foreign companies use a compliance retainer in India</h2>
          </Fade>
                ...
                <div className="flip-wrap">
                  <div className="flip-inner">
                    <div className="flip-front">
                      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "#ccc", marginBottom: 14, fontFamily: HV }}>0{i + 1}</div>
                      <div style={{ fontSize: 16, fontWeight: 800, color: "#111", lineHeight: 1.3, fontFamily: HV, flex: 1 }}>{title}</div>
                      <div className="mob-hide" style={{ paddingTop: 16, fontSize: 11, color: "#bbb", fontFamily: HV, alignItems: "center", gap: 4 }}>
                        <span>Hover to read</span><span style={{ fontSize: 13 }}>→</span>
                      </div>
                    </div>
                    <div className="flip-back">
                      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "rgba(255,255,255,0.35)", marginBottom: 12, fontFamily: HV }}>0{i + 1}</div>
                      <div style={{ fontSize: 15, fontWeight: 800, color: "#fff", lineHeight: 1.2, marginBottom: 12, fontFamily: HV }}>{title}</div>
                      <div style={{ fontSize: 13, color: "rgba(255,255,255,0.82)", lineHeight: 1.75, fontFamily: HV, flex: 1 }}>{body}</div>
                    </div>
                  </div>
                </div>
              </Fade>
      ...
      {/* ── CTA — quote banner matching screenshot ── */}

...
const inp = { width: '100%', padding: '10px 14px', fontSize: 14, border: `1.5px solid ${T.bdr}`, borderRadius: 8, background: '#fff', color: T.ch, fontFamily: 'inherit' };
...
export default function Page() {
  ...
  const Chk = ({ val, set, children }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: T.stone, borderRadius: 8, cursor: 'pointer', marginBottom: 14 }} onClick={() => { set(!val); setRes(null); }}>
      ...
      <span style={{ fontSize: 13, color: T.ink }}>{children}</span>
    </div>
  );

  return (
    <div>
      <section style={{ backgroundImage: "url('/banners and logos/Advance Tax Calculator.png')", backgroundSize: "cover", backgroundPosition: "center", padding: '100px 40px 64px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 860, margin: '0 auto', position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ display: 'inline-block', fontSize: 10, letterSpacing: 3, textTransform: 'uppercase', color: T.sl, fontWeight: 600, marginBottom: 16, padding: '4px 12px', border: '1px solid rgba(245,168,40,.25)', borderRadius: 20 }}>FY 2026-27</div>
          <h1 className="font-display" style={{ fontSize: 'clamp(30px,4vw,52px)', fontWeight: 600, color: '#fff', lineHeight: 1.08, marginBottom: 14 }}>Advance Tax Calculator</h1>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div className="tools-calc-grid">
            <div style={{ background: '#fff', border: `1px solid ${T.bdr}`, borderRadius: 16, padding: '28px 26px' }}>
              <h2 style={{ fontSize: 15, fontWeight: 700, color: T.ch, marginBottom: 20 }}>Inputs</h2>
              <div style={{ marginBottom: 18 }}>
                <div style={{ fontSize: 12.5, fontWeight: 600, color: T.mid, marginBottom: 6 }}>Taxpayer Type</div>
                <div style={{ display: 'flex', gap: 8 }}>
                  {['individual', 'company'].map(t => (
                    <button key={t} onClick={() => { setTab(t); setRes(null); }} style={{ padding: '8px 16px', fontSize: 13, fontWeight: 600, borderRadius: 7, border: 'none', cursor: 'pointer', background: tab === t ? T.f : T.stone, color: tab === t ? '#fff' : T.mid }}>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12.5, fontWeight: 600, color: T.mid, marginBottom: 6 }}>Estimated Total Income FY 2026-27 (₹)</div>
                <input style={inp} type="text" placeholder="e.g. 1500000" value={income} onChange={e => { setIncome(e.target.value); setRes(null); }} />
              </div>
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 12.5, fontWeight: 600, color: T.mid, marginBottom: 6 }}>Expected TDS / TCS for the year (₹) — optional</div>
                <input style={inp} type="text" placeholder="e.g. 50000" value={tds} onChange={e => { setTds(e.target.value); setRes(null); }} />
              </div>
              <div style={{ background: '#FFF8ED', border: '1px solid #F5E2B8', borderRadius: 8, padding: '10px 14px', marginBottom: 18, fontSize: 12, color: '#7A5C1E' }}>
                For accurate tax computation, use the <Link href="/tools/income-tax-calculator" style={{ color: T.f, fontWeight: 600 }}>Income Tax Calculator ↗</Link>
              </div>
              <button onClick={calc} style={{ width: '100%', padding: '13px', fontSize: 14, fontWeight: 700, background: T.f, color: '#fff', border: 'none', borderRadius: 10, cursor: 'pointer' }}>
            ...
            <div>
              {!res ? (
                <div style={{ background: '#fff', border: `1px solid ${T.bdr}`, borderRadius: 16, padding: '48px 28px', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 14, color: T.mid }}>Enter details and click Calculate.</div>
                </div>
              ) : res.exempt ? (
                <div style={{ background: '#E4F0EB', border: `1px solid ${T.f}`, borderRadius: 16, padding: '32px 28px', textAlign: 'center' }}>
                  <div style={{ fontSize: 36, marginBottom: 10 }}>✅</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: T.f, marginBottom: 8 }}>Advance Tax Exempt</div>
                  <p style={{ fontSize: 13, color: T.mid }}>Senior citizens (75+) with only pension/interest income from specified banks are exempt from advance tax under Sec 207.</p>
                </div>
              ) : res.low ? (
                <div style={{ background: '#E4F0EB', border: `1px solid ${T.f}`, borderRadius: 16, padding: '32px 28px', textAlign: 'center' }}>
                  <div style={{ fontSize: 36, marginBottom: 10 }}>✅</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: T.f, marginBottom: 8 }}>No Advance Tax Required</div>
                  <p style={{ fontSize: 13, color: T.mid }}>Net advance tax payable is <strong>{fmt(res.net)}</strong> — below the ₹10,000 threshold. No advance tax is needed.</p>
                </div>
              ) : (
                <div>
                  <div style={{ background: '#fff', border: `1px solid ${T.bdr}`, borderRadius: 14, padding: '18px 20px', marginBottom: 14 }}>
                    {[['Estimated total tax (inc. cess)', fmt(res.tax)], res.tdsAmt > 0 && ['Less: TDS/TCS credit', `(${fmt(res.tdsAmt)})`], ['Net Advance Tax Payable', fmt(res.net)]].filter(Boolean).map(([l, v], i, a) => (
                      <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: i < a.length - 1 ? `1px solid ${T.bdr}` : 'none', fontWeight: i === a.length - 1 ? 700 : 400, fontSize: i === a.length - 1 ? 15 : 13.5 }}>
                  ...
                  {res.pre ? (
                    <div style={{ background: '#FFF8ED', border: '1px solid #F5E2B8', borderRadius: 12, padding: '18px 20px' }}>
                      <div style={{ fontWeight: 700, color: '#7A5C1E', marginBottom: 6 }}>Presumptive Taxation</div>
                      <p style={{ fontSize: 13, color: '#7A5C1E', margin: 0 }}>Entire advance tax of <strong>{fmt(res.net)}</strong> is due in one instalment by <strong>15 March 2027</strong>.</p>
                    </div>
                  ) : (
                      ...
                      {res.insts.map((inst, i) => {
                        ...
                        return (
                          <div key={i} style={{ background: '#fff', border: `1.5px solid ${isNext ? T.f : T.bdr}`, borderRadius: 12, padding: '14px 18px', position: 'relative' }}>
                            {isNext && <div style={{ position: 'absolute', top: -1, right: 12, background: T.f, color: '#fff', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: '0 0 6px 6px' }}>NEXT DUE</div>}
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <div>
                                <div style={{ fontSize: 13, fontWeight: 700, color: T.ch }}>{inst.label} Instalment</div>
                                <div style={{ fontSize: 12, color: T.lt }}>Due: {inst.due} · Cumulative {inst.cumPct}%</div>
                              </div>
                              <div style={{ textAlign: 'right' }}>
                                <div style={{ fontSize: 17, fontWeight: 700, color: isPast ? T.lt : T.f }}>{fmt(inst.cumAmt)}</div>
                                <div style={{ fontSize: 11.5, color: T.s, fontWeight: 600 }}>Pay now: {fmt(inst.thisAmt)}</div>
                              </div>
                            </div>
                          </div>
                        );
                  ...
                  <div style={{ marginTop: 12, fontSize: 12, color: '#7A5C1E', padding: '10px 14px', background: '#FFF8ED', borderRadius: 8, border: '1px solid #F5E2B8' }}>

...
export default function Page() {
  ...
  const catBadge = (c) => {
    ...
    return <span style={{ ...helvetica, fontSize: 10.5, fontWeight: 600, padding: '2px 8px', borderRadius: 12, background: colors.bg, color: colors.color }}>{c}</span>;
  };
  ...
  const newSecCell = (newSec, tableRef) => (
    <div>
      ...
      {tableRef && (
        <div style={{ ...helvetica, fontSize: 10.5, color: T.lt, marginTop: 5, lineHeight: 1.4 }}>{tableRef}</div>
      )}
    </div>
  );
  ...
  return (
    <div style={helvetica}>
      {/* HERO — unchanged */}
      <section style={{ backgroundImage: "url('/banners and logos/TDS RATES (1).webp')", backgroundSize: "cover", backgroundPosition: "center", padding: '100px 40px 64px', position: 'relative', overflow: 'hidden' }}>
      ...
      <section style={{ background: '#fff', padding: '48px 40px 80px' }}>
        <div style={{ maxWidth: 1500, margin: '0 auto' }}>
          ...
          {/* IT Act 2025 section mapping banner */}
              ...
              {CATS.map(c => (
                <button key={c} onClick={() => setCat(c)}
                  style={{
                    ...
                    background: cat === c ? T.f : '#f0f0f0', color: cat === c ? '#fff' : T.mid, transition: 'all .15s'
                  }}>
                  {c}
                </button>
              ))}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1200 }}>
              <thead>
                <tr>
                  ...
                  <th style={{ ...thStyle, width: 140 }}>New Sec (IT 2025)<br />and Table Ref</th>
          ...
          <div style={{ marginTop: 32, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
            {[
              ['Surcharge and Cess', 'For resident payments, TDS is at base rate (no surcharge/cess). For non-resident payments (Sec. 393(2) / old Sec 195 onwards), applicable surcharge and 4% H&E Cess are added on top.'],

...
export default function GSTDueDates() {
  ...
  return (
    <div style={helvetica}>
      {/* ── HERO — unchanged ─────────────────────────────────────────────── */}
      <section style={{ backgroundImage: "url('/banners and logos/GST DUE DATE  (1).webp')", backgroundSize: "cover", backgroundPosition: "center", padding: '100px 40px 64px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', position: 'relative', zIndex: 2 }}>
          ...
          <p style={{ ...helvetica, fontSize: 15, color: 'rgba(255,255,255,.55)', lineHeight: 1.7, maxWidth: 620, marginBottom: 32 }}>
          ...
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            {[
              ...
              <div key={lbl} style={{ background: 'rgba(255,255,255,.08)', borderRadius: 12, padding: '12px 18px', borderLeft: `3px solid ${T.sl}` }}>
                <div style={{ ...helvetica, fontSize: 20, fontWeight: 700, color: '#fff' }}>{val}</div>
                <div style={{ ...helvetica, fontSize: 11, color: 'rgba(255,255,255,.5)', marginTop: 2 }}>{lbl}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ background: '#fff', border: `1px solid ${T.bdr}`, borderRadius: 14, padding: '20px 24px', marginBottom: 28 }}>
            <div style={{ marginBottom: 16 }}>
              <div style={{ ...helvetica, fontSize: 12, fontWeight: 700, color: T.mid, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>Taxpayer Type</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {FILER_TYPES.map(f => (
                  <button key={f} onClick={() => setFilerType(f)}
                    style={{
                      ...helvetica, padding: '8px 16px', fontSize: 13, fontWeight: 600, borderRadius: 8, border: 'none', cursor: 'pointer',
                      background: filerType === f ? T.f : '#f0f0f0', color: filerType === f ? '#fff' : T.mid, transition: 'all .15s'
                    }}>
            ...
            <div>
              <div style={{ ...helvetica, fontSize: 12, fontWeight: 700, color: T.mid, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>Return Type</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {RETURN_TYPES.map(r => {
                  ...
                  return (
                    <button key={r} onClick={() => setReturnType(r)}
                      style={{
                        ...helvetica, padding: '7px 14px', fontSize: 12.5, fontWeight: 600, borderRadius: 8,
                        border: `1.5px solid ${returnType === r ? (c ? c.border : T.f) : T.bdr}`, cursor: 'pointer',
            ...
            <div style={{ ...helvetica, marginTop: 12, fontSize: 12, color: T.lt }}>
          ...
          {(filerType === 'All' || filerType === 'QRMP') && (
            <div style={{ ...helvetica, background: '#FFF8ED', border: '1px solid #F5E2B8', borderRadius: 12, padding: '14px 18px', marginBottom: 24, fontSize: 12.5, color: '#7A5C1E' }}>
          ...
          {/* Grouped list */}
          {grouped.length === 0 ? (
            <div style={{ ...helvetica, textAlign: 'center', padding: '48px 24px', color: T.mid, background: '#fff', borderRadius: 14, border: `1px solid ${T.bdr}` }}>
              No due dates found for the selected filters.
            </div>
          ) : (
            grouped.map(([monthLabel, entries]) => (
              <div key={monthLabel} style={{ marginBottom: 28 }}>
                <div style={{ ...helvetica, fontSize: 13, fontWeight: 700, color: T.mid, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 12, paddingLeft: 4, borderLeft: `3px solid ${T.sl}`, paddingBottom: 2 }}>
                  {monthLabel}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {entries.map((e, i) => {
                    const status = getStatus(e.due);
                    const tc = TYPE_COLORS[e.type] || { bg: '#f0f0f0', color: T.mid, border: T.bdr };
                      ...
                      <div key={i} style={{
                        ...
                        <div style={{ flex: '1 1 300px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4, flexWrap: 'wrap' }}>
                            <span style={{ ...helvetica, padding: '3px 10px', borderRadius: 20, fontSize: 11.5, fontWeight: 700, background: tc.bg, color: tc.color, border: `1px solid ${tc.border}` }}>
                              {e.type}
                            </span>
                            <span style={{ ...helvetica, fontSize: 12.5, fontWeight: 600, color: T.ch }}>{e.period}</span>
                            <span style={{ ...helvetica, padding: '2px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600, background: '#f0f0f0', color: T.mid }}>{e.filer}</span>
                          </div>
                          <div style={{ ...helvetica, fontSize: 12.5, color: T.mid, lineHeight: 1.5 }}>{e.desc}</div>
                        </div>
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          <div style={{ ...helvetica, fontSize: 16, fontWeight: 700, color: status === 'overdue' ? '#DC2626' : status === 'upcoming' ? '#D97706' : T.ch }}>
                            {formatDate(e.due)}
                          </div>
                          <div style={{ ...helvetica, fontSize: 11.5, fontWeight: 600, marginTop: 3, color: status === 'overdue' ? '#DC2626' : status === 'upcoming' ? '#D97706' : T.lt }}>
                            {status === 'overdue'
          ...
          {/* Disclaimer */}
          <div style={{ ...helvetica, marginTop: 32, background: '#f9f9f9', border: `1px solid ${T.bdr}`, borderRadius: 12, padding: '18px 24px', fontSize: 12.5, color: T.mid, lineHeight: 1.65 }}>

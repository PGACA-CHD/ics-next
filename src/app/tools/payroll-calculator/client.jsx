...
export default function PayrollCalculator() {
  ...
  function calculate() {
    ...
    setResult({
      ...
      totalDeductions, netAnnual, netMonthly, taxableForIT,
    });
  }

  const inputStyle = {
    ...helvetica, width: '100%', padding: '10px 14px', fontSize: 14,
    border: `1.5px solid ${T.bdr}`, borderRadius: 8, background: '#fff',
    color: T.ch, boxSizing: 'border-box',
  };
  ...
  return (
    <div style={helvetica}>
      {/* HERO — unchanged */}
      <section style={{ backgroundImage: "url('/banners and logos/Payroll.webp')", backgroundSize: "cover", backgroundPosition: "center", padding: '100px 40px 64px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', position: 'relative', zIndex: 2 }}>
          <Link href="/tools" style={{ ...helvetica, fontSize: 12.5, color: 'rgba(255,255,255,.45)', marginBottom: 18, display: 'inline-block', textDecoration: 'none' }}>← Back to Tools</Link>
          ...
          <h1 className="font-display" style={{ ...helvetica, fontSize: 'clamp(28px,4vw,52px)', fontWeight: 600, color: '#fff', lineHeight: 1.08, marginBottom: 14 }}>
            Payroll Calculator
          </h1>
          <p style={{ ...helvetica, fontSize: 15, color: 'rgba(255,255,255,.55)', lineHeight: 1.7, maxWidth: 620 }}>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div className="tools-calc-grid">
            ...
            <div style={{ background: '#fff', border: `1px solid ${T.bdr}`, borderRadius: 16, padding: '32px 28px' }}>
              <h2 style={{ ...helvetica, fontSize: 16, fontWeight: 700, color: T.ch, marginBottom: 24 }}>Salary Inputs</h2>
              ...
              <div style={{ marginBottom: 22 }}>
                ...
                <div style={{ ...helvetica, display: 'flex', justifyContent: 'space-between', fontSize: 11, color: T.lt, marginTop: 4 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {[['metro', 'Metro (Delhi/Mumbai/Chennai/Kolkata)'], ['nonmetro', 'Non-Metro']].map(([v, l]) => (
                    <button key={v} onClick={() => { setCityType(v); setResult(null); }}
                      style={{
                        ...helvetica, flex: 1, padding: '9px 10px', fontSize: 12.5, fontWeight: 600, borderRadius: 8,
              ...
              <button onClick={calculate}
                style={{ ...helvetica, width: '100%', padding: '13px', fontSize: 14, fontWeight: 700, background: T.f, color: '#fff', border: 'none', borderRadius: 10, cursor: 'pointer' }}>
            ...
            <div>
              {!result ? (
                <div style={{ background: '#fff', border: `1px solid ${T.bdr}`, borderRadius: 16, padding: '48px 28px', textAlign: 'center' }}>
                  ...
                  <div style={{ ...helvetica, fontSize: 15, color: T.mid }}>Enter CTC details and click<br /><strong style={{ color: T.ch }}>"Calculate Take-Home Salary"</strong></div>
                </div>
              ) : (
                <PayrollResult result={result} regime={regime} hraPct={hraPct} employeePF={result.employeePF} />
              )}
            </div>
          </div>
...
function PayrollResult({ result, regime, hraPct, employeePF }) {
  const {
    annualCTC, basicAnnual, hraAnnual, specialAllowance, employerPF, gratuity,
    grossAnnual, grossMonthly,
    esiEmployee, esiApplicable, ptAnnual, incomeTax, hraExempt,
    totalDeductions, netAnnual, netMonthly,
  } = result;

  const tableRow = (label, annual, note) => (
    <tr style={{ borderBottom: `1px solid ${T.bdr}` }}>
      <td style={{ ...helvetica, padding: '9px 14px', fontSize: 13, color: T.mid }}>{label}{note && <div style={{ ...helvetica, fontSize: 11, color: T.lt, marginTop: 2 }}>{note}</div>}</td>
      <td style={{ ...helvetica, padding: '9px 14px', fontSize: 13, fontWeight: 500, color: T.ch, textAlign: 'right' }}>{fmt(annual)}</td>
      <td style={{ ...helvetica, padding: '9px 14px', fontSize: 13, color: T.mid, textAlign: 'right' }}>{fmt(annual / 12)}</td>
    </tr>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ background: T.f, borderRadius: 16, padding: '22px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <div style={{ ...helvetica, fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,.5)', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 4 }}>Net Take-Home (Monthly)</div>
          <div style={{ ...helvetica, fontSize: 32, fontWeight: 700, color: '#fff' }}>{fmt(netMonthly)}</div>
          <div style={{ ...helvetica, fontSize: 13, color: 'rgba(255,255,255,.6)', marginTop: 4 }}>{fmt(netAnnual)} / year · {pct}% of CTC</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ ...helvetica, fontSize: 12, color: 'rgba(255,255,255,.5)', marginBottom: 4 }}>Gross Monthly</div>
          <div style={{ ...helvetica, fontSize: 20, fontWeight: 600, color: '#fff' }}>{fmt(grossMonthly)}</div>
          <div style={{ ...helvetica, fontSize: 12, color: 'rgba(255,255,255,.5)', marginTop: 4 }}>Gross Annual: {fmt(grossAnnual)}</div>
        </div>
      </div>

      {/* CTC Breakdown */}
      <div style={{ background: '#fff', border: `1px solid ${T.bdr}`, borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ background: '#f5f5f5', padding: '12px 16px', borderBottom: `1px solid ${T.bdr}` }}>
          <span style={{ ...helvetica, fontSize: 13, fontWeight: 700, color: T.ch }}>CTC Breakdown</span>
          <span style={{ ...helvetica, float: 'right', fontSize: 12, color: T.lt }}>Annual &nbsp;|&nbsp; Monthly</span>
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#f9f9f9' }}>
              <th style={{ ...helvetica, padding: '8px 14px', fontSize: 11.5, fontWeight: 700, color: T.lt, textAlign: 'left', textTransform: 'uppercase', letterSpacing: 0.8 }}>Component</th>
              <th style={{ ...helvetica, padding: '8px 14px', fontSize: 11.5, fontWeight: 700, color: T.lt, textAlign: 'right', textTransform: 'uppercase', letterSpacing: 0.8 }}>Annual</th>
              <th style={{ ...helvetica, padding: '8px 14px', fontSize: 11.5, fontWeight: 700, color: T.lt, textAlign: 'right', textTransform: 'uppercase', letterSpacing: 0.8 }}>Monthly</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td colSpan={3} style={{ ...helvetica, padding: '6px 14px', fontSize: 10.5, fontWeight: 700, color: T.lt, background: '#fafafa', textTransform: 'uppercase', letterSpacing: 0.9, borderBottom: `1px solid ${T.bdr}` }}>
            ...
            <tr style={{ background: '#EDF6F0', borderTop: `1.5px solid ${T.f}` }}>
              <td style={{ ...helvetica, padding: '10px 14px', fontSize: 13.5, fontWeight: 700, color: T.f }}>Gross Salary</td>
              <td style={{ ...helvetica, padding: '10px 14px', fontSize: 13.5, fontWeight: 700, color: T.f, textAlign: 'right' }}>{fmt(grossAnnual)}</td>
              <td style={{ ...helvetica, padding: '10px 14px', fontSize: 13.5, fontWeight: 700, color: T.f, textAlign: 'right' }}>{fmt(grossMonthly)}</td>
            </tr>
            ...
            <tr style={{ background: '#f5f5f5', borderTop: `2px solid ${T.ch}` }}>
              <td style={{ ...helvetica, padding: '10px 14px', fontSize: 13.5, fontWeight: 700, color: T.ch }}>Total Annual CTC</td>
              <td style={{ ...helvetica, padding: '10px 14px', fontSize: 13.5, fontWeight: 700, color: T.ch, textAlign: 'right' }}>{fmt(annualCTC)}</td>
              <td style={{ ...helvetica, padding: '10px 14px', fontSize: 13.5, fontWeight: 700, color: T.ch, textAlign: 'right' }}>{fmt(annualCTC / 12)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div style={{ background: '#fff', border: `1px solid ${T.bdr}`, borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ background: '#f5f5f5', padding: '12px 16px', borderBottom: `1px solid ${T.bdr}` }}>
          <span style={{ ...helvetica, fontSize: 13, fontWeight: 700, color: T.ch }}>Deductions</span>
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <tbody>
            {tableRow('Employee PF', result.employeePF, '12% of Basic, max ₹21,600/yr')}
            {esiApplicable
              ? tableRow('ESI (Employee)', esiEmployee, '0.75% of Gross — applicable (gross ≤ ₹21,000/month)')
              : <tr style={{ borderBottom: `1px solid ${T.bdr}` }}>
                <td style={{ ...helvetica, padding: '9px 14px', fontSize: 13, color: T.lt }}>ESI (Employee)<div style={{ ...helvetica, fontSize: 11, marginTop: 2 }}>Not applicable — gross monthly &gt; ₹21,000</div></td>
                <td style={{ ...helvetica, padding: '9px 14px', fontSize: 13, color: T.lt, textAlign: 'right' }}>N/A</td>
                <td style={{ ...helvetica, padding: '9px 14px', fontSize: 13, color: T.lt, textAlign: 'right' }}>N/A</td>
              </tr>
            }
            ...
            {regime === 'old' && hraExempt > 0 && (
              <tr style={{ borderBottom: `1px solid ${T.bdr}` }}>
                <td style={{ ...helvetica, padding: '9px 14px', fontSize: 13, color: '#2E7D32' }}>HRA Exemption (−ve, reduces taxable income)<div style={{ ...helvetica, fontSize: 11, color: '#4CAF50', marginTop: 2 }}>Min of: Actual HRA, Rent − 10% Basic, {hraPct >= 50 ? '50% Basic (metro)' : '40% Basic (non-metro)'}</div></td>
                <td style={{ ...helvetica, padding: '9px 14px', fontSize: 13, fontWeight: 500, color: '#2E7D32', textAlign: 'right' }}>({fmt(hraExempt)})</td>
                <td style={{ ...helvetica, padding: '9px 14px', fontSize: 13, color: '#2E7D32', textAlign: 'right' }}>({fmt(hraExempt / 12)})</td>
              </tr>
            )}
            {tableRow('Income Tax (incl. 4% cess)', incomeTax, `${regime === 'new' ? 'New Regime, std. ded. ₹75,000' : 'Old Regime, std. ded. ₹50,000, 80C ₹' + Math.round(Math.min(result.employeePF, 150000)).toLocaleString('en-IN')}`)}
            <tr style={{ background: '#FFF0F0', borderTop: `2px solid #DC2626` }}>
              <td style={{ ...helvetica, padding: '10px 14px', fontSize: 13.5, fontWeight: 700, color: '#DC2626' }}>Total Deductions</td>
              <td style={{ ...helvetica, padding: '10px 14px', fontSize: 13.5, fontWeight: 700, color: '#DC2626', textAlign: 'right' }}>{fmt(totalDeductions)}</td>
              <td style={{ ...helvetica, padding: '10px 14px', fontSize: 13.5, fontWeight: 700, color: '#DC2626', textAlign: 'right' }}>{fmt(totalDeductions / 12)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div style={{ background: '#fff', border: `1px solid ${T.bdr}`, borderRadius: 14, padding: '18px 22px' }}>
        <div style={{ ...helvetica, fontSize: 13, fontWeight: 700, color: T.ch, marginBottom: 14 }}>CTC Composition</div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
          {components.map(c => (
            <div key={c.label} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
              ...
              <span style={{ ...helvetica, color: T.mid }}>{c.label}</span>
              <span style={{ ...helvetica, fontWeight: 600, color: T.ch }}>{fmt(c.val / 12)}/mo</span>
            </div>
          ))}
        </div>
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${T.bdr}`, display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
          <span style={{ ...helvetica, color: T.mid }}>CTC Utilisation (take-home / CTC)</span>
          <span style={{ ...helvetica, fontWeight: 700, color: T.ch }}>{pct}%</span>
        </div>
      </div>
    </div>
  );

...
<section style={{ background: T.f, padding: "110px 56px 72px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <button className="ics-btn ics-btn-primary ics-btn-lg" onClick={() => { window.location.href = ROUTES["contact"] || "/"; }}>Book Free Consultation →</button>
            <button className="ics-btn ics-btn-ghost ics-btn-lg" onClick={() => { window.location.href = ROUTES["services"] || "/"; }}>View Entity Types →</button>
          </div>
        </div>
      </section>
      {/* Content */}
  ...
  return (
    <div style={{ background: T.stone, borderRadius: 16, padding: "28px 30px", border: `1px solid ${T.bdr}`, marginBottom: 32 }}>
      ...
      <div style={{ background: "#E4F0EB", borderRadius: 8, padding: "10px 14px", display: "flex", gap: 8 }}>
        <span style={{ color: T.f, fontWeight: 700, fontSize: 13, flexShrink: 0 }}>v</span>
            ...
            <div key={r.route} style={{ background: r.light, border: `1px solid ${r.color}30`, borderRadius: 14, padding: "22px 20px" }}>
              ...
              {r.points.map(p => (
                <div key={p} style={{ display: "flex", gap: 8, marginBottom: 6 }}>
                  <span style={{ color: r.color, fontWeight: 700, flexShrink: 0 }}>v</span>
                  <span style={{ fontSize: 13, color: T.mid, lineHeight: 1.55 }}>{p}</span>
                </div>
              ))}
              {r.warning && <div style={{ marginTop: 12, background: "#FEF3C7", borderRadius: 7, padding: "8px 12px", fontSize: 12, color: "#92400E", fontWeight: 600 }}>! {r.warning}</div>}
            </div>
          ))}
        </div>
      </SEOSection>
    ...
    <SEOPageLayout eyebrow="India Market Entry . Ex-KPMG Advisory" setPage={setPage}
      ...
      <SEOSection label="Cost & Timeline" title="What does foreign company registration in India cost?">
        <div style={{ background: T.stone, borderRadius: 14, padding: "28px 30px", border: `1px solid ${T.bdr}`, marginBottom: 16 }}>
          {[
            { item: "Government filing fees (MCA + RBI)", range: "Rs.5,000 – Rs.15,000", note: "Depends on authorised share capital" },
            { item: "Professional fees – incorporation", range: "Rs.50,000 – Rs.1,50,000", note: "Varies by complexity, foreign director requirements" },
            { item: "FCGPR + FEMA filing", range: "Rs.25,000 – Rs.50,000", note: "Included in our engagement" },
            { item: "Transfer pricing policy", range: "Rs.40,000 – Rs.80,000", note: "Strongly recommended at incorporation" },
            { item: "Typical total (standard case)", range: "Rs.1.5 – 3 lakhs", note: "All-in, including TP documentation" },
          ].map(r => (
      ...
      <SEOSection label="FAQ" title="Frequently asked questions">
        <SEOFAQs items={[
          ...
          { q: "What is the minimum paid-up capital required?", a: "There is no minimum paid-up capital requirement for a Private Limited Company in India. In practice, most foreign companies subscribe between Rs.1 lakh and Rs.10 lakhs to demonstrate commitment to the Indian subsidiary." },
    ...
    <SEOPageLayout eyebrow="India Market Entry . Strategic Advisory" setPage={setPage}
      ...
      <SEOSection label="What We Advise On" title="The full scope of India market entry advisory">
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:8 }} className="seo-2col">
          {[
            { icon:"🏛", title:"Entity structure selection", desc:"WOS, Branch, Liaison, LLP, JV – the right choice depends on your sector, FDI route, tax objective, and long-term India plans. We model all options before recommending one." },
            { icon:"🛣", title:"FDI route & FEMA compliance", desc:"Automatic vs Government Route, sector eligibility, investment caps, FEMA filing obligations. Getting the route wrong invalidates the entire investment." },
            { icon:"⚖", title:"DTAA planning & treaty benefits", desc:"Which DTAA applies, what rates apply to your specific payment flows, how to structure intercompany transactions to legitimately minimise WHT." },
    ...
    <SEOPageLayout eyebrow="NRI . India Business Setup . FEMA Advisory" setPage={setPage}
      ...
      <SEOSection label="Two Types of NRI Clients" title="Which situation applies to you?">
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:8 }} className="seo-2col">
          {[
            {
              icon:"✈", type:"NRI investing from abroad",
            ...
            <div key={c.type} style={{ background:c.color, border:`1px solid ${T.bdr}`, borderRadius:14, padding:"24px 22px" }}>
              ...
              {c.points.map(p => (
                <div key={p} style={{ display:"flex", gap:8, marginBottom:7 }}>
                  <span style={{ color:T.s, fontWeight:700, flexShrink:0, fontSize:12 }}>v</span>
  ...
  return (
    <SEOPageLayout eyebrow="Company Formation . India . CA-Led" setPage={setPage}
      ...
      <SEOSection label="Key Advantages" title="Why a Private Limited Company is the right choice">
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }} className="seo-2col">
          {[
            { icon:"🛡", title:"Limited liability", desc:"Shareholders are liable only for the unpaid amount on their shares. Personal assets are protected from company debts." },
      ...
      <SEOSection label="Cost & Timeline" title="What Private Limited Company registration costs">
        <div style={{ background:T.stone, borderRadius:14, padding:"28px 30px", border:`1px solid ${T.bdr}`, marginBottom:16 }}>
          {[
            { item:"Government fees (MCA + stamp duty)", range:"Rs.2,000 – Rs.15,000", note:"Based on authorised share capital and state" },
            { item:"Professional fees – incorporation", range:"Rs.15,000 – Rs.40,000", note:"Name approval, DSC, SPICe+, MOA/AOA, bank account" },
            { item:"GST registration", range:"Rs.3,000 – Rs.6,000", note:"If applicable – included in our standard package" },
            { item:"Auditor appointment", range:"Rs.3,000 – Rs.5,000", note:"ADT-1 filing – mandatory within 30 days" },
            { item:"Total all-in (standard case)", range:"Rs.25,000 – Rs.60,000", note:"Domestic founders, two directors, standard sector" },
          ].map(r => (
            <div key={r.item} style={{ display:"grid", gridTemplateColumns:"1fr auto", gap:16, padding:"12px 0", borderBottom:`1px solid ${T.bdr}`, alignItems:"start" }}>
              <div>
                <div style={{ fontSize:13.5, fontWeight:600, color:T.ch }}>{r.item}</div>
      ...
      <SEOSection label="Common Mistakes" title="What founders get wrong at incorporation">
        <SEOMistakes items={[
          ...
          { title:"Under-capitalising at setup", desc:"Authorised share capital affects stamp duty – many founders minimise it to save Rs.2,000 in stamp duty, then face problems when raising the first investor round because the authorised capital is too low to issue shares." },
        ...
        <div style={{ background:"#FFF8F0", border:"1px solid #FFE0B2", borderRadius:12, padding:"20px 22px", marginTop:16 }}>
          ...
          {[
            ...
            <div key={i} style={{ display:"flex", gap:10, marginBottom:8 }}>
              <span style={{ color:"#B45309", fontWeight:700, flexShrink:0 }}>v</span>
    ...
    <SEOPageLayout eyebrow="Entity Structure . India Entry" setPage={setPage}
      ...
      <SEOSection label="Why a Subsidiary" title="Why most foreign companies choose a subsidiary over other structures">
        <div className="seo-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
          {[
            { icon:"OK", title:"Full commercial operations", desc:"Can earn revenue, sign contracts, hire employees across all functions." },
            { icon:"OK", title:"Lower tax rate", desc:"25.17% corporate tax vs 40% for a branch office. Significant savings at scale." },
            { icon:"OK", title:"Automatic FDI route", desc:"In most sectors – no prior Government approval needed to incorporate." },
            { icon:"OK", title:"Limited liability", desc:"Parent is not liable for the subsidiary's debts beyond its share subscription." },
            { icon:"OK", title:"Fundraising capability", desc:"Can issue equity to investors, employees (ESOPs), and strategic partners." },
            { icon:"OK", title:"Repatriation of profits", desc:"Dividends can be repatriated to the parent subject to withholding tax and DTAA." },
          ].map(b => (
      ...
      <SEOSection label="The Process" title="How to set up a wholly owned subsidiary in India">
        <SEOSteps steps={[
          ...
          { title: "Decide share capital & structure", time: "Days 1 - 2", desc: "Determine authorised and paid-up share capital. For most subsidiaries, Rs.1 - 10 lakh is sufficient to start. Capital structure affects stamp duty and future fundraising." },
      ...
      <SEOSection label="Cost & Timeline" title="Cost of setting up a subsidiary in India">
        <div style={{ background: T.stone, borderRadius: 14, padding: "28px 30px", border: `1px solid ${T.bdr}`, marginBottom: 16 }}>
          {[
            { item: "MCA incorporation fees", range: "Rs.2,000 – Rs.10,000", note: "Based on authorised capital" },
            { item: "Professional fees – incorporation", range: "Rs.50,000 – Rs.1,20,000", note: "Including MOA/AOA drafting, DSC, DIN, SPICe+" },
            { item: "FEMA/FCGPR filing", range: "Rs.25,000 – Rs.40,000", note: "Included in our standard engagement" },
            { item: "Transfer pricing policy", range: "Rs.40,000 – Rs.75,000", note: "Highly recommended at incorporation stage" },
            { item: "Nominee director (first year)", range: "Rs.20,000 – Rs.40,000", note: "If required" },
            { item: "Total all-in (standard case)", range: "Rs.1.5 – 2.5 lakhs", note: "Typical end-to-end including TP" },
          ].map(r => (
            <div key={r.item} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 16, padding: "12px 0", borderBottom: `1px solid ${T.bdr}`, alignItems: "start" }}>
              <div>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: T.ch }}>{r.item}</div>
    ...
    <SEOPageLayout eyebrow="International Tax . Transfer Pricing" setPage={setPage}
      ...
      <SEOSection label="Common Mistakes" title="Transfer pricing mistakes Indian subsidiaries make">
        <SEOMistakes items={[
          ...
          { title: "Missing the Form 3CEB deadline", desc: "Form 3CEB must be filed by 31 October. Missing this deadline attracts a penalty of Rs.1 lakh under Section 271BA, regardless of whether the TP position is correct." },
        ]}/>
      </SEOSection>
      ...
      <SEOSection label="Cost & Fees" title="What transfer pricing compliance costs in India">
        <div style={{ background: T.stone, borderRadius: 14, padding: "28px 30px", border: `1px solid ${T.bdr}`, marginBottom: 16 }}>
          {[
            { item: "TP study + Form 3CEB (simple)", range: "Rs.40,000 – Rs.75,000", note: "Single transaction type, TNMM methodology" },
            { item: "TP study (multi-transaction)", range: "Rs.75,000 – Rs.1,50,000", note: "Multiple transaction types, complex benchmarking" },
            { item: "Annual TP retainer", range: "Rs.60,000 – Rs.1,20,000/year", note: "Study + 3CEB + ongoing monitoring" },
            { item: "TP audit representation", range: "Rs.1,00,000 – Rs.3,00,000", note: "Responding to scrutiny notices, DRP filings" },
            { item: "APA application", range: "Rs.3,00,000 – Rs.8,00,000", note: "Unilateral APA – 5-year pricing certainty" },
          ].map(r => (
            <div key={r.item} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 16, padding: "12px 0", borderBottom: `1px solid ${T.bdr}`, alignItems: "start" }}>
              <div>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: T.ch }}>{r.item}</div>

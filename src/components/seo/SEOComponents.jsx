        ...
        <div style={{ maxWidth:1400, margin:"0 auto" }}>
          <div style={{ background:T.stone, borderRadius:14, padding:"24px 28px", border:`1px solid ${T.bdr}` }}>
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:10 }}>
              {[
                { label:"Foreign company registration in India â", page:"seo_fcri" },
                { label:"Wholly owned subsidiary setup â", page:"seo_sub" },
                { label:"FDI rules & automatic route â", page:"seo_fdi" },
                { label:"Transfer pricing requirements â", page:"seo_tp" },
              ].map(l => (
...
<section style={{ background: T.f, padding: "110px 56px 72px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <button className="ics-btn ics-btn-primary ics-btn-lg" onClick={() => (() => { if(typeof window !== "undefined") window.location.href = ROUTES["contact"] || "/"; })()}>Book Free Consultation â</button>
            <button className="ics-btn ics-btn-ghost ics-btn-lg" onClick={() => (() => { if(typeof window !== "undefined") window.location.href = ROUTES["services"] || "/"; })()}>View Entity Types â</button>
          </div>
        </div>
      </section>
      {/* Content */}
  ...
  return (
    <div style={{ background: T.stone, borderRadius: 16, padding: "28px 30px", border: `1px solid ${T.bdr}`, marginBottom: 32 }}>
      ...
      <div style={{ background: "#E4F0EB", borderRadius: 8, padding: "10px 14px", display: "flex", gap: 8 }}>
        <span style={{ color: T.f, fontWeight: 700, fontSize: 13, flexShrink: 0 }}>â</span>
...
// âââ SEO PAGE 1: FOREIGN COMPANY REGISTRATION INDIA ââââââââââââââââââââââââââ



function SEOFDIRulesPage({ setPage }) {
  return (
    <SEOPageLayout eyebrow="FDI & FEMA Compliance Â· India Entry" setPage={setPage}
      title={<>FDI Rules in India â<br/><span style={{ fontStyle:"italic", color:T.sl }}>What Foreign Investors Must Know</span></>}
      description="India's FDI policy governs how foreign companies can invest in India. Here is a plain-English guide to FDI routes, sector caps, prohibited sectors, and FEMA compliance â with what actually matters in practice.">

      <SEOSection label="Overview" title="India's FDI framework â how it works">
        <SEOProseP>Foreign Direct Investment (FDI) in India is regulated under the Foreign Exchange Management Act (FEMA), administered by the Reserve Bank of India (RBI), with sector-specific policy issued by the Department for Promotion of Industry and Internal Trade (DPIIT). India received USD 70.97 billion in FDI in FY 2023â24, making it one of the world's top FDI destinations.</SEOProseP>
        ...
        <SEOProseP>Getting FDI compliance wrong creates serious exposure. Investments made without proper authorisation, incorrect route classification, or missed FEMA filings attract penalties up to three times the transaction value â with RBI having power to compel regularisation.</SEOProseP>
      </SEOSection>

      <SEOSection label="The Two Routes" title="Automatic Route vs. Government Route â the critical distinction">
        <div className="seo-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
          {[
            ...
            { route: "Government Route", color: "#B45309", light: "#FFF8F0", title: "Prior FIPB/Cabinet approval required", points: ["Application filed through DPIIT portal", "Timeline: 8â12 weeks for approval", "Required for defence, insurance, broadcasting, telecom, multi-brand retail", "Some sectors have mandatory conditions (e.g., local sourcing)"], warning: "Investment before approval is a FEMA violation" },
          ].map(r => (
            <div key={r.route} style={{ background: r.light, border: `1px solid ${r.color}30`, borderRadius: 14, padding: "22px 20px" }}>
              ...
              {r.points.map(p => (
                <div key={p} style={{ display: "flex", gap: 8, marginBottom: 6 }}>
                  <span style={{ color: r.color, fontWeight: 700, flexShrink: 0 }}>â</span>
                  <span style={{ fontSize: 13, color: T.mid, lineHeight: 1.55 }}>{p}</span>
                </div>
              ))}
              {r.warning && <div style={{ marginTop: 12, background: "#FEF3C7", borderRadius: 7, padding: "8px 12px", fontSize: 12, color: "#92400E", fontWeight: 600 }}>â ï¸ {r.warning}</div>}
            </div>
          ))}
        </div>
      </SEOSection>
      ...
      <SEOSection label="Sector Caps" title="Key sector FDI limits at a glance">
        ...
        <p style={{ fontSize: 12.5, color: T.lt }}>This is a simplified summary. FDI policy changes periodically â always verify current rules before proceeding.</p>
      </SEOSection>

      <SEOSection label="FEMA Compliance" title="Mandatory FEMA filings after FDI">
        <SEOSteps steps={[
          { title: "FC-GPR (Foreign Currency â Gross Provisional Return)", time: "Within 30 days", desc: "Filed with the RBI through the FIRMS portal after shares are allotted to the foreign investor. Declares the amount of FDI received, the number of shares allotted, and the valuation basis. This is the most important FEMA filing â missing the 30-day deadline requires compounding." },
      ...
      <SEOSection label="Real Client Example" title="FDI compliance in practice">
        <SEOClientStory flag="ðºð¦" region="Europe Â· Manufacturing Group Â· Long-established India entity"
          headline="European manufacturer regularised 3 years of missed FLA Returns"
          challenge="The India subsidiary had been filing its tax returns correctly but had missed FLA Returns for three consecutive years â the finance team in Europe was unaware of this RBI requirement."
      ...
      <SEOSection label="Common Mistakes" title="FDI and FEMA mistakes foreign companies make">
        <SEOMistakes items={[
          ...
          { title: "Missing the FC-GPR 30-day window", desc: "This is the most common FEMA violation we encounter. Many companies complete incorporation correctly but miss the FC-GPR filing deadline because they are unaware of it. A compounding application must then be filed â adding months of delay and regulatory cost." },
          { title: "Not filing FLA Returns annually", desc: "The Annual FLA Return is not filed through MCA â it is filed directly with RBI and is separate from all other annual compliances. Many companies are unaware of it until they receive a notice." },
        ]}/>
      </SEOSection>

      <SEOSection label="FAQ" title="FDI and FEMA questions">
        <SEOFAQs items={[
          { q: "Can a foreign company own 100% of an Indian company?", a: "Yes â in most sectors. 100% FDI under the Automatic Route is permitted in manufacturing, IT, most services, and e-commerce (marketplace model). Restricted sectors such as insurance (74% cap), defence (74%), and multi-brand retail (51%) have lower limits." },
          { q: "What is the difference between FEMA and FDI policy?", a: "FDI policy (issued by DPIIT) determines what foreign investment is permitted in which sectors and at what ownership levels. FEMA (administered by RBI) governs how the investment is executed â the forms, timelines, and compliance obligations. Both apply simultaneously and must be complied with." },
          { q: "What is compounding under FEMA?", a: "Compounding is the process by which a FEMA violation is regularised with the RBI. It involves admitting the violation, paying a compounding fee, and obtaining an order regularising the breach. Most missed FC-GPR and FLA filings are resolved through compounding. The fee varies â typically ranging from the interest amount to a fixed penalty depending on the nature and duration of the violation." },
...
// âââ ARTICLE PAGE (individual Knowledge Hub articles) âââââââââââââââââââââââââ

function SEOForeignCompanyPage({ setPage }) {
  return (
    <SEOPageLayout eyebrow="India Market Entry Â· Ex-KPMG Advisory" setPage={setPage}
      title={<>Foreign Company Registration<br/><span style={{ fontStyle:"italic", color:T.sl }}>in India â Complete Guide</span></>}
      description="How to register a foreign company in India â entity types, FDI routes, RBI filings, and compliance timelines. Ex-KPMG CA team. 100+ foreign companies registered.">

      <SEOSection label="What It Means" title="Foreign company registration in India â what it actually involves">
        <SEOProseP>Registering a foreign company in India is not a single form â it is a multi-step process involving the Ministry of Corporate Affairs (MCA), the Reserve Bank of India (RBI), and in most cases, the GST department and income tax authorities. The process differs significantly depending on the entity type you choose and the FDI route applicable to your sector.</SEOProseP>
        <SEOProseP>The right structure must be decided before any filing begins. Choosing incorrectly â for example, setting up a branch office in a sector that requires a subsidiary â creates expensive restructuring work later. Getting the transfer pricing model wrong at incorporation means years of audit exposure.</SEOProseP>
      ...
      <SEOSection label="Your Options" title="Four ways a foreign company can be registered in India">
        <div className="seo-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 8 }}>
          {[
            { type: "Private Limited Company (WOS)", tax: "25.17%", fdi: "Automatic in most sectors", revenue: "Full commercial", best: "Most foreign companies â full operations, fundraising, hiring" },
            ...
            { type: "Branch Office", tax: "40%", fdi: "RBI approval required", revenue: "Limited â only parent's activities", best: "Exporting goods/services, research only" },
            { type: "Liaison Office", tax: "Nil", fdi: "RBI approval required", revenue: "None â no commercial activity", best: "Market research, promoting parent company only" },
          ].map(e => (
      ...
      <SEOSection label="The Process" title="Step-by-step: how foreign company registration works in India">
        <SEOSteps steps={[
          ...
          { title: "Digital Signature Certificates (DSC)", time: "Days 2â3", desc: "All proposed directors require DSCs. For foreign nationals, this requires passport copy, address proof, and notarisation. We handle the filing." },
          { title: "Director Identification Number (DIN)", time: "Days 3â5", desc: "Each director requires a DIN from MCA. For foreign directors, we file Form DIR-3 with apostilled documents." },
          { title: "Name reservation via RUN", time: "Days 4â6", desc: "Company name is reserved through MCA's RUN (Reserve Unique Name) system. We check trademark conflicts and regulatory restrictions before submission." },
          { title: "SPICe+ filing with MCA", time: "Days 6â14", desc: "The main incorporation form â includes MOA, AOA, registered office, PAN, TAN, and GSTIN application. Certificate of Incorporation typically issues within 7â12 working days of document submission." },
          { title: "RBI FCGPR filing", time: "Within 30 days of share allotment", desc: "Foreign Currency Gross Provisional Return â mandatory for all foreign investment under FEMA. We file via the RBI's FIRMS portal." },
          { title: "Bank account & post-setup", time: "Weeks 3â5", desc: "Current account with an Indian bank, GST registration, TDS registration, payroll setup, and a compliance calendar handed over ready to use." },
        ]}/>
      </SEOSection>

      <SEOSection label="Real Client Example" title="How it works in practice">
        <SEOClientStory flag="ðºð¸" region="USA Â· SaaS Company Â· Series B"
          headline="Cloud analytics platform registered in India in 19 days"
          challenge="The company needed an India entity before their first engineering hire arrived in Bangalore. Time pressure was significant â payroll had to be live within the month."
          outcome="Private limited company incorporated, FCGPR filing completed, transfer pricing policy documented, first payroll run â all within 30 days of engagement."
      ...
      <SEOSection label="Common Mistakes" title="What foreign companies get wrong">
        <SEOMistakes items={[
          ...
          { title: "Not setting up transfer pricing before the first intercompany transaction", desc: "Transfer pricing documentation is legally required from the first payment between the India entity and its foreign parent. Companies that get this wrong at incorporation face back-audits and penalties up to 2Ã the underpaid tax." },
          { title: "Missing the FCGPR 30-day filing window", desc: "FEMA requires the FCGPR to be filed within 30 days of share allotment. Missing this deadline requires a compounding application â a formal RBI regularisation process that takes months and attracts penalties." },
      ...
      <SEOSection label="Cost & Timeline" title="What does foreign company registration in India cost?">
        <div style={{ background: T.stone, borderRadius: 14, padding: "28px 30px", border: `1px solid ${T.bdr}`, marginBottom: 16 }}>
          {[
            { item: "Government filing fees (MCA + RBI)", range: "â¹5,000 â â¹15,000", note: "Depends on authorised share capital" },
            { item: "Professional fees â incorporation", range: "â¹50,000 â â¹1,50,000", note: "Varies by complexity, foreign director requirements" },
            { item: "FCGPR + FEMA filing", range: "â¹25,000 â â¹50,000", note: "Included in our engagement" },
            { item: "Transfer pricing policy", range: "â¹40,000 â â¹80,000", note: "Strongly recommended at incorporation" },
            { item: "Typical total (standard case)", range: "â¹1.5 â 3 lakhs", note: "All-in, including TP documentation" },
          ].map(r => (
        ...
        <p style={{ fontSize: 13, color: T.lt }}>Timeline: 19â30 working days for a standard case. Sectors requiring Government approval take 6â12 weeks longer.</p>
      </SEOSection>

      <SEOSection label="FAQ" title="Frequently asked questions">
        <SEOFAQs items={[
          { q: "Can a foreign company own 100% of an Indian company?", a: "Yes â in most sectors, 100% FDI is permitted under the Automatic Route. This means no prior Government approval is needed. Restricted sectors (defence, insurance, media) have lower caps. We check FDI eligibility as the first step of every engagement." },
          { q: "Does the company need an Indian director?", a: "Yes. At least one director of the Indian company must be a resident of India (present in India for at least 182 days in the previous calendar year). This can be a nominee director â we can help arrange one if needed." },
          { q: "What is the minimum paid-up capital required?", a: "There is no minimum paid-up capital requirement for a Private Limited Company in India. In practice, most foreign companies subscribe between â¹1 lakh and â¹10 lakhs to demonstrate commitment to the Indian subsidiary." },
          { q: "How long does registration take?", a: "For a straightforward Private Limited Company with Indian directors and no restricted sector issues, incorporation typically takes 14â21 working days from document submission. The Certificate of Incorporation from MCA issues in 7â12 working days once the SPICe+ is filed." },
          ...
          { q: "Do we need a physical office in India?", a: "Yes â a registered office address in India is mandatory for incorporation. This can be a virtual/registered address initially. Many clients start with a virtual office and upgrade to physical space once the team grows." },
        ]}/>
      </SEOSection>
...
// âââ SEO PAGE 2: SUBSIDIARY COMPANY IN INDIA âââââââââââââââââââââââââââââââââ

function SEOGCCSetupPage({ setPage }) {
  return (
    <SEOPageLayout eyebrow="GCC & Captive Centres Â· India Advisory" setPage={setPage}
      title={<>GCC Setup in India â<br/><span style={{ fontStyle:"italic", color:T.sl }}>End-to-End Advisory</span></>}
      description="Setting up a Global Capability Centre in India requires more than incorporation. Entity structure, cost-plus pricing, ESOP design, payroll, and ongoing compliance â all from one firm.">

      <SEOSection label="What Is a GCC" title="Global Capability Centres in India â what's involved">
        <SEOProseP>A Global Capability Centre (GCC) â also called a captive centre or shared services centre â is an India entity wholly owned by a foreign company that provides services back to the parent. Unlike a branch or liaison office, a GCC is a full private limited company with its own employees, payroll, compliance calendar, and intercompany service agreement with the foreign parent.</SEOProseP>
        <SEOProseP>India hosts over 1,700 GCCs employing more than 1.7 million professionals. The most common functions: software engineering, data analytics, finance & accounting shared services, and legal/compliance operations. The average GCC takes 6â8 weeks to become fully operational from first engagement.</SEOProseP>
        <SEOProseP>The commercial model is typically cost-plus: the India GCC invoices the foreign parent for all its costs plus a mark-up (typically 8â15%). This mark-up is the taxable profit in India. Getting the cost-plus model and transfer pricing documentation right at setup is critical â it determines your India tax liability for the life of the entity.</SEOProseP>
      </SEOSection>

      <SEOSection label="What We Handle" title="Full GCC setup â what's included">
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }} className="seo-2col">
          {[
            { icon:"ð¢", title:"Entity incorporation", desc:"Private Limited Company â SPICe+ filing, MOA/AOA, registered office, PAN, TAN, GSTIN. Typically 14â21 days." },
            { icon:"ð°", title:"Cost-plus pricing model", desc:"Intercompany service agreement, cost allocation methodology, benchmarked mark-up, and annual TP documentation." },
            { icon:"ð¥", title:"HR & payroll setup", desc:"Payroll structure, PF, ESI, professional tax, TDS on salary â compliant from hire #1." },
            { icon:"ð", title:"ESOP structuring", desc:"Employee stock option plan design, FEMA compliance for foreign parent ESOPs, tax optimisation for employees." },
            { icon:"ð", title:"Ongoing compliance retainer", desc:"Monthly GST, TDS, payroll processing. Annual audit, ITR, Form 3CEB, FLA Return. Fixed fee." },
            { icon:"ð", title:"Transfer pricing documentation", desc:"Annual TP study, benchmarking analysis, Form 3CEB certification. Defensible against scrutiny." },
          ].map(s => (
      ...
      <SEOSection label="The Process" title="GCC setup timeline â week by week">
        <SEOSteps steps={[
          { title:"Structure & pricing design", time:"Week 1", desc:"Entity type, FDI route, cost-plus mark-up methodology, DTAA analysis. Intercompany service agreement drafted. This is the most important step â getting the commercial model right before incorporation." },
          { title:"Incorporation", time:"Weeks 1â3", desc:"SPICe+ filing, Certificate of Incorporation, PAN, TAN, GSTIN. Registered office established. Bank account initiation." },
          ...
          { title:"HR & payroll infrastructure", time:"Weeks 3â5", desc:"PF registration, ESI (if applicable), professional tax, TDS on salary. Payroll software setup. Offer letter templates aligned with India employment law." },
          { title:"ESOP plan (if applicable)", time:"Weeks 4â6", desc:"Board resolution, ESOP trust deed (for trust structure) or direct grant, FEMA compliance for foreign parent options. Employee communication plan." },
      ...
      <SEOSection label="Real Client Example" title="GCC setup in practice">
        <SEOClientStory flag="ð¸ð¬" region="Singapore Â· APAC SaaS Â· Series C"
          headline="APAC SaaS company scaled to a 40-person GCC in 8 weeks"
          challenge="Company needed to move from 0 to 40 engineers in Pune. Entity, payroll, ESOP trust, cost-plus pricing, and compliance all required simultaneously â with a hard deadline driven by an upcoming fundraising round."
      ...
      <SEOSection label="Common Mistakes" title="What GCC setups get wrong">
        <SEOMistakes items={[
          ...
          { title:"ESOP documentation gaps", desc:"Foreign parent ESOPs for Indian employees require FEMA compliance at every stage â grant, vesting, exercise, and remittance. Missing FEMA filings at exercise create compounding liability that can be costly to regularise." },
...
// âââ INDIA MARKET ENTRY ADVISORY PAGE ââââââââââââââââââââââââââââââââââââââââ

function SEOMarketEntryPage({ setPage }) {
  return (
    <SEOPageLayout eyebrow="India Market Entry Â· Strategic Advisory" setPage={setPage}
      ...
      <SEOSection label="What Is India Market Entry Advisory" title="More than incorporation â a strategic starting point">
        <SEOProseP>India market entry advisory covers everything that happens before the first document is filed. The entity type, FDI route, transfer pricing model, DTAA structure, and intercompany framework must be decided in the right sequence â because getting any one of them wrong creates restructuring work that typically costs 3â5Ã more than getting it right at inception.</SEOProseP>
        ...
        <SEOProseP>Our advisory work is led by ex-KPMG International Tax professionals â the same people who advise large multinationals on India entry. The difference is that we work with mid-market companies at fees that match mid-market reality.</SEOProseP>
      </SEOSection>
      ...
      <SEOSection label="What We Advise On" title="The full scope of India market entry advisory">
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:8 }} className="seo-2col">
          {[
            { icon:"ðï¸", title:"Entity structure selection", desc:"WOS, Branch, Liaison, LLP, JV â the right choice depends on your sector, FDI route, tax objective, and long-term India plans. We model all options before recommending one." },
            { icon:"ð£ï¸", title:"FDI route & FEMA compliance", desc:"Automatic vs Government Route, sector eligibility, investment caps, FEMA filing obligations. Getting the route wrong invalidates the entire investment." },
            { icon:"âï¸", title:"DTAA planning & treaty benefits", desc:"Which DTAA applies, what rates apply to your specific payment flows, how to structure intercompany transactions to legitimately minimise WHT." },
            { icon:"ð", title:"Transfer pricing framework", desc:"Designing the intercompany pricing model before the first transaction â management fees, royalties, cost allocations. This is the highest-value intervention in the entire entry process." },
            { icon:"ð", title:"PE risk assessment", desc:"Permanent Establishment risk analysis â what activities in India create a tax liability for the foreign parent. Particularly critical for companies with Indian sales staff, advisory relationships, or contract-signing authority in India." },
            { icon:"ð", title:"Compliance architecture", desc:"What filings are required, at what frequencies, by which deadlines. Building the compliance calendar before the entity is operational so nothing is missed from month one." },
          ].map(s => (
            <div key={s.title} style={{ background:"#fff", border:`1px solid ${T.bdr}`, borderRadius:12, padding:"20px 18px", display:"flex", gap:14 }}>
              <span style={{ fontSize:24, flexShrink:0 }}>{s.icon}</span>
              <div>
      ...
      <SEOSection label="Who This Is For" title="The companies we work with">
        <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
          {[
            { audience:"Foreign companies entering India for the first time", desc:"You've decided to enter India. You need to know which entity to form, which FDI route applies to you, and what your transfer pricing obligations will be. We give you this picture in the first consultation â before any money is committed." },
            ...
            { audience:"GCC / captive centre buildout", desc:"You're setting up a 10â500 person India team to provide services to your global operations. The pricing model, entity structure, ESOP design, and compliance framework all need to be right from the start." },
          ].map((item, i) => (
      ...
      <SEOSection label="Our Approach" title="How we work â structure before filing">
        <SEOSteps steps={[
          ...
          { title:"Written structure memo", time:"After the call", desc:"A short written summary of our recommendations â entity type, FDI route, DTAA considerations, transfer pricing approach, and next steps. This memo costs nothing. It's the deliverable from the free consultation." },
          { title:"Detailed advisory engagement", time:"Week 1â2", desc:"If you proceed, we produce a full India entry advisory report â entity comparison, FDI route analysis, TP methodology recommendation, DTAA analysis, compliance obligations schedule, and fee proposal for implementation." },
          { title:"Implementation", time:"Weeks 2â6", desc:"We handle incorporation, FEMA filings, transfer pricing documentation, GST registration, payroll setup, and everything required to make the entity operational." },
      ...
      <SEOSection label="FAQ" title="India market entry questions">
        <SEOFAQs items={[
          { q:"When should I start the India market entry advisory process?", a:"Ideally 3â6 months before you plan to have your first India employee or generate your first India revenue. The structural decisions take time to document and execute correctly. Starting late forces shortcuts that create compliance exposure." },
          { q:"What does India market entry advisory cost?", a:"The initial 30-minute consultation is free. A full India entry advisory report â structure, FDI route, DTAA analysis, TP framework, compliance calendar â typically costs â¹75,000ââ¹1,50,000 depending on complexity. Implementation (incorporation + FEMA + TP documentation) is additional and separately quoted." },
          { q:"Can you advise on India entry if we already have an entity that was set up incorrectly?", a:"Yes â this is a significant part of our work. We assess existing structures for FDI compliance, transfer pricing gaps, FEMA violations, and entity type mismatch. Where issues are found, we design and implement a remediation plan including any necessary RBI compounding applications." },
          ...
          { q:"What makes your India market entry advisory different from a Big 4 firm?", a:"The depth of analysis is equivalent â our lead advisor spent 8 years doing exactly this work at KPMG. The difference is fees and accessibility. Big 4 India market entry engagements for mid-market companies typically cost â¹5â15 lakhs. Our equivalent engagement costs â¹1â3 lakhs. And every engagement is led by a senior CA, not staffed to a junior team." },
        ]}/>
      </SEOSection>
...
// âââ PRIVATE LIMITED COMPANY REGISTRATION âââââââââââââââââââââââââââââââââââââ

function SEONRIPage({ setPage }) {
  return (
    <SEOPageLayout eyebrow="NRI Â· India Business Setup Â· FEMA Advisory" setPage={setPage}
      title={<>NRI Company Registration<br/><span style={{ fontStyle:"italic", color:T.sl }}>in India â Complete Guide</span></>}
      description="NRIs can incorporate a company in India or invest in an existing one. Two routes, different compliance. FEMA Schedule 4, repatriation rules, and residency transition â explained plainly.">
      ...
      <SEOSection label="Two Types of NRI Clients" title="Which situation applies to you?">
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:8 }} className="seo-2col">
          {[
            {
              icon:"âï¸", type:"NRI investing from abroad",
              desc:"You live and work outside India. You want to invest in or incorporate a company in India â for a business you'll manage remotely or through a local team.",
              points:["Investment under Schedule 4 of FEMA (NRI route)", "No RBI FCGPR required â different from FDI route", "Can repatriate dividends freely after tax", "Can be director of an India company while residing abroad"],
              color: T.stone,
            },
            {
              icon:"ð¡", type:"NRI returning to India",
              desc:"You are returning to India to start or run a business. Your residency status is changing â from NRI to Resident. This triggers specific FEMA obligations.",
              points:["Residency transition â NRI â Resident Indian", "Existing foreign assets must be declared under FEMA", "India income now fully taxable from year of return", "Bank accounts: NRE/FCNR â Resident accounts within required timeframe"],
              color:"#fff",
            },
          ].map(c => (
            <div key={c.type} style={{ background:c.color, border:`1px solid ${T.bdr}`, borderRadius:14, padding:"24px 22px" }}>
              ...
              {c.points.map(p => (
                <div key={p} style={{ display:"flex", gap:8, marginBottom:7 }}>
                  <span style={{ color:T.s, fontWeight:700, flexShrink:0, fontSize:12 }}>â</span>
      ...
      <SEOSection label="NRI Investment Route" title="How NRI investment in India works â Schedule 4 FEMA">
        <SEOProseP>NRI investment in Indian companies is governed by Schedule 4 of the Foreign Exchange Management (Non-Debt Instruments) Rules, 2019 â not the FDI regulations that apply to foreign companies. This is a separate and simpler route: no RBI FCGPR filing, no valuation certificate requirement, and no sectoral cap complications in most cases.</SEOProseP>
        <SEOProseP>An NRI can invest in an Indian Private Limited Company by subscribing to shares at incorporation or purchasing shares from existing shareholders. The investment must be made from an NRE (Non-Resident External) or FCNR (Foreign Currency Non-Resident) account â not from an NRO account (unless specific conditions are met).</SEOProseP>
        <SEOSteps steps={[
          ...
          { title:"Company incorporation or share acquisition", time:"Days 2â14", desc:"For a new company: SPICe+ filing with NRI as shareholder. For investment in an existing company: share transfer or fresh allotment, updated share register, Form SH-4 or MGT-7 as applicable." },
          { title:"Bank account and fund remittance", time:"Days 10â20", desc:"Investment must be received from NRE/FCNR account or inward remittance. Bank account opened in company name. Share capital deposited." },
          { title:"Post-investment compliance", time:"Within 30â60 days", desc:"For certain NRI investments, intimation to authorised dealer bank is required. We handle the documentation and ensure the investment is properly recorded under the correct FEMA schedule." },
        ]}/>
      </SEOSection>

      <SEOSection label="NRI Returning to India" title="FEMA transition â what changes when you return">
        <div style={{ background:T.stone, borderRadius:14, padding:"28px 30px", border:`1px solid ${T.bdr}`, marginBottom:8 }}>
          ...
          {[
            ...
            { title:"Tax residency change", desc:"In the year of return, you may be 'Resident but Not Ordinarily Resident' (RNOR) â a transitional status that provides a 2-year window where foreign income may not be taxable in India. Proper tax planning before return is essential." },
          ].map((item, i) => (
      ...
      <SEOSection label="Real Client Example" title="NRI setup in practice">
        <SEOClientStory flag="ðºð¸" region="USA Â· NRI Entrepreneur Â· Returning to India"
      ...
      <SEOSection label="Common Mistakes" title="What NRIs get wrong">
        <SEOMistakes items={[
          ...
          { title:"Not planning residency transition before returning", desc:"NRIs who return to India without planning the transition often convert NRE accounts too early, lose RNOR tax status benefit, or fail to properly declare foreign assets. The planning should happen 3â6 months before physical return." },
          { title:"Mixing NRI investment route with FDI route documentation", desc:"NRI investment under Schedule 4 does not require RBI FCGPR filing. But many NRIs and their advisors prepare FDI documentation unnecessarily â creating confusion at future funding rounds about the nature of the investment." },
      ...
      <SEOSection label="FAQ" title="NRI company registration questions">
        <SEOFAQs items={[
          ...
          { q:"What happens to my NRE account when I return to India?", a:"When you become a Resident Indian, NRE accounts must be re-designated as RFC (Resident Foreign Currency) accounts or converted to regular resident savings accounts. The funds in the NRE account at the time of conversion can be retained freely â there is no requirement to repatriate them." },
...
// âââ STARTUP FOREIGN INVESTMENT ââââââââââââââââââââââââââââââââââââââââââââââââ

function SEOPvtLtdPage({ setPage }) {
  return (
    <SEOPageLayout eyebrow="Company Formation Â· India Â· CA-Led" setPage={setPage}
      title={<>Private Limited Company<br/><span style={{ fontStyle:"italic", color:T.sl }}>Registration in India</span></>}
      description="Register a Private Limited Company in India â SPICe+ filing, MOA/AOA, PAN, TAN, GST, and post-incorporation compliance. CA-led. FDI-ready from day one. 7â12 working days.">

      <SEOSection label="What It Is" title="Private Limited Company â India's most versatile business structure">
        <SEOProseP>A Private Limited Company is the most widely used business structure in India. Governed by the Companies Act, 2013 and regulated by the Ministry of Corporate Affairs (MCA), it provides limited liability, separate legal identity, and the ability to raise funding â making it the default choice for startups, growing businesses, and foreign-owned Indian entities alike.</SEOProseP>
        ...
        <SEOProseP>Our difference from low-cost incorporation platforms: we are a CA firm. We don't just file the MCA forms â we advise on the right MOA objects clause, share capital structure, director appointments, and post-incorporation compliance calendar before the Certificate of Incorporation is issued. Getting this right at inception saves months of amendment work later.</SEOProseP>
      </SEOSection>

      <SEOSection label="Key Advantages" title="Why a Private Limited Company is the right choice">
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }} className="seo-2col">
          {[
            { icon:"ð¡ï¸", title:"Limited liability", desc:"Shareholders are liable only for the unpaid amount on their shares. Personal assets are protected from company debts." },
            { icon:"ð°", title:"FDI and fundraising ready", desc:"Accepts foreign investment under Automatic Route in most sectors. Can issue equity to angel investors and VCs without restriction." },
            { icon:"ð", title:"ESOP capability", desc:"Can issue Employee Stock Options to build and retain a team. ESOPs require a board resolution â we set this up at incorporation if needed." },
            { icon:"ð", title:"Perpetual succession", desc:"The company continues to exist regardless of changes in shareholders or directors. Not dependent on any individual." },
            { icon:"ð¦", title:"Credibility with banks and vendors", desc:"Corporate customers, banks, and government agencies prefer dealing with a Pvt Ltd over a proprietorship or partnership." },
            { icon:"ð", title:"Tax efficiency", desc:"Corporate tax rate of 25.17% (22% base + surcharge + cess) for domestic companies. Lower than partnership tax in many cases." },
          ].map(b => (
      ...
      <SEOSection label="The Process" title="How Private Limited Company registration works">
        <SEOSteps steps={[
          { title:"Name reservation via RUN", time:"Days 1â2", desc:"We check trademark conflicts, MCA name availability, and regulatory restrictions. Name reserved through MCA's RUN (Reserve Unique Name) system. We advise on the MOA objects clause â getting this right determines what your company can legally do." },
          { title:"Digital Signature Certificates (DSC)", time:"Days 2â3", desc:"All proposed directors require DSCs. For Indian nationals, this takes 1â2 working days. For foreign nationals, apostilled documents are required." },
          { title:"Director Identification Number (DIN)", time:"Days 3â4", desc:"Each director requires a DIN from MCA. For new directors with no existing DIN, we apply through SPICe+ simultaneously." },
          { title:"SPICe+ filing with MCA", time:"Days 4â12", desc:"The main incorporation form â MOA, AOA, registered office address, PAN, TAN, and GSTIN applied simultaneously. Certificate of Incorporation (CIN) issued in 7â12 working days of document submission." },
          { title:"Bank account & post-incorporation setup", time:"Days 12â20", desc:"Current account opened. Share capital deposited. GST registration completed. Auditor appointed. Compliance calendar handed over." },
        ]}/>
      </SEOSection>

      <SEOSection label="What We Handle" title="Complete incorporation â not just MCA filing">
        <div style={{ background:T.stone, borderRadius:14, padding:"28px 30px", border:`1px solid ${T.bdr}` }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:0 }} className="seo-2col">
            {[
              ...
              ["DSC for all directors", "Indian and foreign nationals â we handle apostille coordination"],
              ["SPICe+ filing", "MCA incorporation form â PAN, TAN, GSTIN in one shot"],
              ...
              ["Auditor appointment", "Form ADT-1 within 30 days â statutory requirement"],
      ...
      <SEOSection label="Cost & Timeline" title="What Private Limited Company registration costs">
        <div style={{ background:T.stone, borderRadius:14, padding:"28px 30px", border:`1px solid ${T.bdr}`, marginBottom:16 }}>
          {[
            { item:"Government fees (MCA + stamp duty)", range:"â¹2,000 â â¹15,000", note:"Based on authorised share capital and state" },
            { item:"Professional fees â incorporation", range:"â¹15,000 â â¹40,000", note:"Name approval, DSC, SPICe+, MOA/AOA, bank account" },
            { item:"GST registration", range:"â¹3,000 â â¹6,000", note:"If applicable â included in our standard package" },
            { item:"Auditor appointment", range:"â¹3,000 â â¹5,000", note:"ADT-1 filing â mandatory within 30 days" },
            { item:"Total all-in (standard case)", range:"â¹25,000 â â¹60,000", note:"Domestic founders, two directors, standard sector" },
          ].map(r => (
            <div key={r.item} style={{ display:"grid", gridTemplateColumns:"1fr auto", gap:16, padding:"12px 0", borderBottom:`1px solid ${T.bdr}`, alignItems:"start" }}>
              <div>
                <div style={{ fontSize:13.5, fontWeight:600, color:T.ch }}>{r.item}</div>
        ...
        <p style={{ fontSize:13, color:T.lt }}>Timeline: 7â12 working days for standard cases. Cases involving foreign shareholders or restricted sectors take longer.</p>
      </SEOSection>

      <SEOSection label="Common Mistakes" title="What founders get wrong at incorporation">
        <SEOMistakes items={[
          { title:"Wrong objects clause in MOA", desc:"The MOA objects clause determines what your company can legally do. Founders often choose a generic clause and then find it doesn't cover their actual business activity â requiring a costly MGT-14 amendment. We advise on the objects clause before filing." },
          { title:"Under-capitalising at setup", desc:"Authorised share capital affects stamp duty â many founders minimise it to save â¹2,000 in stamp duty, then face problems when raising the first investor round because the authorised capital is too low to issue shares." },
          ...
          { title:"Ignoring FDI-readiness even for domestic companies", desc:"If there's any chance of a foreign investor or co-founder joining later, the initial structure should be FDI-ready â right sector classification, clear shareholding pattern, no issue that would create complications under FEMA at a later stage." },
        ]}/>
      </SEOSection>

      <SEOSection label="FAQ" title="Private Limited Company registration â common questions">
        <SEOFAQs items={[
          ...
          { q:"Is there a minimum paid-up capital requirement?", a:"No. A Private Limited Company can be incorporated with any amount of paid-up capital â even â¹10,000. Authorised share capital of â¹1 lakh divided into 10,000 shares of â¹10 each is the standard starting point. Paid-up capital is the amount actually invested by shareholders." },
          { q:"Can a foreign national be a director or shareholder?", a:"Yes. Foreign nationals can be directors and shareholders. At least one director must be an Indian resident. Foreign shareholders investing in the company must comply with FEMA's FDI regulations â share allotment triggers an RBI FCGPR filing requirement." },
          { q:"How long does registration take?", a:"7â12 working days from the date of document submission with MCA. Name approval takes 2â3 working days. DSC procurement for foreign nationals can add 5â7 days if apostille is required." },
          ...
          { q:"Do I need a physical office address at the time of incorporation?", a:"Yes â a registered office address is mandatory. It can be a residential address with a No-Objection Certificate from the property owner. Many founders use a virtual office or co-working address initially and upgrade to a commercial space later." },
        ]}/>
      </SEOSection>
...
// âââ NRI COMPANY REGISTRATION âââââââââââââââââââââââââââââââââââââââââââââââââ

function SEOStartupFDIPage({ setPage }) {
  return (
    <SEOPageLayout eyebrow="Indian Startups Â· Foreign Funding Â· FEMA Advisory" setPage={setPage}
      title={<>Indian Startup Receiving<br/><span style={{ fontStyle:"italic", color:T.sl }}>Foreign Investment</span></>}
      description="FEMA compliance, angel tax, valuation, convertible instruments â everything an Indian startup needs to handle when receiving its first foreign investment. CA advisory from term sheet to allotment.">

      <SEOSection label="What This Covers" title="Foreign investment in Indian startups â the full picture">
        <SEOProseP>When an Indian startup receives investment from a foreign investor â angel, VC, strategic â it triggers FEMA compliance obligations that most founders are unprepared for. The investment is foreign direct investment (FDI) regardless of the amount, and must be structured, valued, and reported correctly.</SEOProseP>
          ...
          {[
            {
              ...
              desc:"The standard instrument for foreign VC investment in Indian startups. Preference shares that mandatorily convert to equity at a future date. FEMA compliant â treated as FDI from day one. Allows for liquidation preference, anti-dilution, and investor rights. Must be valued by a CA using DCF or NAV methodology before allotment.",
            ...
            },
            {
              ...
              desc:"Popular in US startup ecosystem but has uncertain FEMA status in India. A SAFE is not equity and not debt â RBI has not issued clear guidance. Most advisors recommend converting SAFEs to CCPS before any FEMA reporting obligation arises. We advise on the correct treatment for your specific situation.",
              fema:"Uncertain â typically treat as debt (ECB) until conversion", tax:"Angel tax risk on conversion â valuation must be documented"
            },
            {
      ...
      <SEOSection label="Angel Tax" title="Angel tax â what startups must know in 2025â26">
        <SEOProseP>Angel tax (Section 56(2)(viib) of the Income Tax Act) applies when a private company issues shares to a resident investor at a price exceeding the fair market value (FMV) of those shares. The excess is treated as income of the company and taxed at 30%+. A 2023 amendment extended angel tax to foreign investors â creating compliance obligations for Indian startups raising from foreign angels.</SEOProseP>
        <SEOProseP>The Finance Act 2024 has provided significant relief â angel tax does not apply to investments from DPIIT-recognised startups, and certain foreign investor categories (SEBI-registered VCs, Category I/II AIFs, and certain specified entities) are exempt. For founders not covered by an exemption, valuation documentation is critical.</SEOProseP>
        <div style={{ background:"#FFF8F0", border:"1px solid #FFE0B2", borderRadius:12, padding:"20px 22px", marginTop:16 }}>
          ...
          {[
            "Get DPIIT startup recognition â this is the most complete exemption",
            "Obtain a Rule 11UA valuation (DCF method) before share allotment â if shares are issued at or below FMV, no angel tax",
            "Foreign investors who are VCs, AIFs, or from CBDT-notified countries are exempt",
            "Issue CCPS rather than equity where possible â preference shares have different FMV calculation methods",
            "Document the valuation methodology contemporaneously â don't reconstruct it later",
          ].map((p, i) => (
            <div key={i} style={{ display:"flex", gap:10, marginBottom:8 }}>
              <span style={{ color:"#B45309", fontWeight:700, flexShrink:0 }}>â</span>
      ...
      <SEOSection label="The Process" title="From term sheet to share allotment â step by step">
        <SEOSteps steps={[
          { title:"Term sheet review and structure advice", time:"Before signing", desc:"We review the term sheet for FEMA compliance, instrument selection, valuation methodology, and angel tax exposure. The term sheet stage is when structural changes are easiest â before legal documentation is drafted." },
          { title:"DPIIT recognition (if not already obtained)", time:"2â4 weeks", desc:"DPIIT startup recognition provides complete angel tax exemption and simplifies future regulatory filings. We advise on eligibility and help with the application." },
          { title:"Valuation certificate (Rule 11UA)", time:"Before allotment", desc:"A registered CA must certify the FMV of shares using DCF methodology before shares are allotted to foreign investors. This valuation must be contemporaneous â cannot be backdated." },
          ...
          { title:"Share allotment and share certificates", time:"Day of close", desc:"Shares allotted, share certificates issued. The clock starts for FCGPR filing â 30 days from allotment date." },
      ...
      <SEOSection label="Real Client Example" title="Startup foreign funding in practice">
        <SEOClientStory flag="ð®ð³" region="India Â· B2B SaaS Startup Â· Seed Round"
          headline="Indian SaaS startup structured a $500K seed round from US angels â FEMA compliant, angel tax nil"
      ...
      <SEOSection label="FAQ" title="Foreign investment in startups â questions">
        <SEOFAQs items={[
          { q:"Does my startup need to be DPIIT registered to receive foreign investment?", a:"No â DPIIT registration is not required to receive foreign investment. However, DPIIT recognition provides complete exemption from angel tax under Section 56(2)(viib), which is a significant benefit. For startups raising from foreign angels or investors not covered by the angel tax exemption, DPIIT recognition is strongly recommended." },
...
function SEOSubsidiaryPage({ setPage }) {
  return (
    <SEOPageLayout eyebrow="Entity Structure Â· India Entry" setPage={setPage}
      title={<>Setting Up a Subsidiary<br/><span style={{ fontStyle:"italic", color:T.sl }}>Company in India</span></>}
      description="A wholly owned subsidiary (WOS) is the most common structure for foreign companies entering India. Here is exactly how to set one up correctly â legal structure, FDI route, RBI compliance, and transfer pricing.">

      <SEOSection label="What Is a Subsidiary" title="Subsidiary vs. branch office vs. liaison office â what's the difference?">
        <SEOProseP>A subsidiary company in India is a separate Indian legal entity â typically a Private Limited Company â where the foreign parent holds 100% (or majority) of the shares. It is the most common structure for foreign companies entering India for full commercial operations.</SEOProseP>
        ...
        <SEOProseP>A wholly owned subsidiary (WOS) is a subsidiary where the foreign parent holds 100% of shares â the standard for most foreign companies entering India.</SEOProseP>
      </SEOSection>
      ...
      <SEOSection label="Why a Subsidiary" title="Why most foreign companies choose a subsidiary over other structures">
        <div className="seo-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
          {[
            { icon:"â", title:"Full commercial operations", desc:"Can earn revenue, sign contracts, hire employees across all functions." },
            { icon:"â", title:"Lower tax rate", desc:"25.17% corporate tax vs 40% for a branch office. Significant savings at scale." },
            { icon:"â", title:"Automatic FDI route", desc:"In most sectors â no prior Government approval needed to incorporate." },
            { icon:"â", title:"Limited liability", desc:"Parent is not liable for the subsidiary's debts beyond its share subscription." },
            { icon:"â", title:"Fundraising capability", desc:"Can issue equity to investors, employees (ESOPs), and strategic partners." },
            { icon:"â", title:"Repatriation of profits", desc:"Dividends can be repatriated to the parent subject to withholding tax and DTAA." },
          ].map(b => (
      ...
      <SEOSection label="The Process" title="How to set up a wholly owned subsidiary in India">
        <SEOSteps steps={[
          ...
          { title: "Decide share capital & structure", time: "Days 1â2", desc: "Determine authorised and paid-up share capital. For most subsidiaries, â¹1â10 lakh is sufficient to start. Capital structure affects stamp duty and future fundraising." },
          { title: "Appoint Indian resident director", time: "Days 1â5", desc: "At least one director must be an Indian resident. This can be a nominee director if your team is entirely overseas. We help arrange nominees where needed." },
          { title: "Document preparation & DSC", time: "Days 3â7", desc: "Draft MOA and AOA, obtain Digital Signature Certificates for all directors, prepare apostilled parent company documents." },
          { title: "SPICe+ filing & Certificate of Incorporation", time: "Days 7â19", desc: "The consolidated MCA filing â covers company name, directors, registered office, PAN, TAN, and GSTIN. Certificate of Incorporation typically issues within 7â12 working days." },
          { title: "FEMA & RBI compliance", time: "Within 30 days of share allotment", desc: "File FC-GPR (Form Foreign Currency â Gross Provisional Return) with RBI through the FIRMS portal. Mandatory for all foreign investment. Missing this 30-day window requires a penalty compounding application." },
          { title: "Post-incorporation setup", time: "Weeks 4â6", desc: "Bank account, GST registration, TDS registration, payroll setup, compliance calendar. Full operational readiness." },
        ]}/>
      </SEOSection>

      <SEOSection label="Real Client Example" title="Subsidiary setup in practice">
        <SEOClientStory flag="ð¬ð§" region="UK Â· Fintech Company Â· FCA Regulated"
      ...
      <SEOSection label="Common Mistakes" title="What goes wrong with subsidiary setups">
        <SEOMistakes items={[
          { title: "No transfer pricing policy at incorporation", desc: "Every intercompany transaction between the subsidiary and parent â management fees, royalties, service charges â requires transfer pricing documentation under Indian law. Most companies set this up a year later, after transactions have already occurred, creating back-audit exposure." },
      ...
      <SEOSection label="Cost & Timeline" title="Cost of setting up a subsidiary in India">
        <div style={{ background: T.stone, borderRadius: 14, padding: "28px 30px", border: `1px solid ${T.bdr}`, marginBottom: 16 }}>
          {[
            { item: "MCA incorporation fees", range: "â¹2,000 â â¹10,000", note: "Based on authorised capital" },
            { item: "Professional fees â incorporation", range: "â¹50,000 â â¹1,20,000", note: "Including MOA/AOA drafting, DSC, DIN, SPICe+" },
            { item: "FEMA/FCGPR filing", range: "â¹25,000 â â¹40,000", note: "Included in our standard engagement" },
            { item: "Transfer pricing policy", range: "â¹40,000 â â¹75,000", note: "Highly recommended at incorporation stage" },
            { item: "Nominee director (first year)", range: "â¹20,000 â â¹40,000", note: "If required" },
            { item: "Total all-in (standard case)", range: "â¹1.5 â 2.5 lakhs", note: "Typical end-to-end including TP" },
          ].map(r => (
            <div key={r.item} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 16, padding: "12px 0", borderBottom: `1px solid ${T.bdr}`, alignItems: "start" }}>
              <div>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: T.ch }}>{r.item}</div>
        ...
        <p style={{ fontSize: 13, color: T.lt }}>Timeline: 19â25 working days for a standard case. Sectors needing Government approval take 8â12 weeks longer.</p>
      </SEOSection>
      ...
      <SEOSection label="FAQ" title="Frequently asked questions about subsidiary setup">
        <SEOFAQs items={[
          ...
          { q: "Can the subsidiary hire employees immediately after incorporation?", a: "Yes â once incorporation is complete and a bank account is opened, the subsidiary can hire employees and run payroll. GST registration and TDS registration should be in place before the first payment cycle." },
          { q: "What is an FCGPR and when does it need to be filed?", a: "FC-GPR (Foreign Currency Gross Provisional Return) is the mandatory RBI filing for foreign investment. It must be filed within 30 days of share allotment. Missing this deadline requires a compounding application with the RBI â a formal regularisation process with penalties." },
          { q: "Can the subsidiary pay dividends to the foreign parent?", a: "Yes. After paying corporate tax and complying with Companies Act distribution requirements, a subsidiary can declare dividends to the foreign parent. Dividends are subject to withholding tax â typically 20% under Indian domestic law, reduced under applicable DTAA." },
...
// âââ SEO PAGE 3: TRANSFER PRICING INDIA ââââââââââââââââââââââââââââââââââââââ

function SEOTransferPricingPage({ setPage }) {
  return (
    <SEOPageLayout eyebrow="International Tax Â· Transfer Pricing" setPage={setPage}
      title={<>Transfer Pricing in India â<br/><span style={{ fontStyle:"italic", color:T.sl }}>A Practical Guide</span></>}
      ...
      <SEOSection label="What Is Transfer Pricing" title="Transfer pricing in India â what it means for foreign companies">
        <SEOProseP>Transfer pricing is the set of rules governing how prices are set for transactions between related companies â for example, between your India subsidiary and its foreign parent. The Indian Income Tax Act (Section 92â92F) requires that all such transactions be priced at arm's length â i.e., at the same price two unrelated parties would agree on.</SEOProseP>
      ...
      <SEOSection label="Compliance Requirements" title="What transfer pricing compliance looks like in practice">
        <SEOSteps steps={[
          ...
          { title: "Benchmark analysis", time: "Annually", desc: "A comparability analysis using CMIE ProwessIQ or TP Catalyst databases â comparing your entity's margins against industry benchmarks. This is the core of the TP study and requires specialist expertise." },
      ...
      <SEOSection label="Real Client Example" title="Transfer pricing done right â and wrong">
        <SEOClientStory flag="ð¦ðª" region="UAE Â· Manufacturing Group Â· 12-Year-Old India Entity"
      ...
      <SEOSection label="Common Mistakes" title="Transfer pricing mistakes Indian subsidiaries make">
        <SEOMistakes items={[
          { title: "Setting up transfer pricing after transactions have already occurred", desc: "The most common and costly mistake. TP documentation must be in place before the first intercompany payment. Reconstructing it retroactively is possible but creates risk â auditors give less weight to documentation prepared after the fact." },
          { title: "Underpaying or overpaying on management fees without benchmarking", desc: "Management fees charged by the parent to the subsidiary must be benchmarked against what an unrelated party would pay for equivalent services. Rates that are too high (over-charging the India entity) reduce taxable income in India â exactly what TP auditors look for." },
          { title: "Treating GCC entities as cost centres without proper cost-plus documentation", desc: "GCCs providing services to the foreign parent at cost are still subject to TP rules. A cost-plus methodology with a mark-up (typically 8â15%) must be documented, benchmarked, and defended." },
          { title: "Missing the Form 3CEB deadline", desc: "Form 3CEB must be filed by 31 October. Missing this deadline attracts a penalty of â¹1 lakh under Section 271BA, regardless of whether the TP position is correct." },
        ]}/>
      </SEOSection>

      <SEOSection label="Audit Risk" title="How India's transfer pricing audit system works">
        <SEOProseP>India has one of the most active transfer pricing audit regimes in Asia. Cases are selected for TP scrutiny based on risk parameters set by the Central Board of Direct Taxes (CBDT) â typically companies with large international transactions, significant adjustments in prior years, or sectors known for TP disputes (IT services, pharma, financial services).</SEOProseP>
        <SEOProseP>A TP adjustment â where the tax officer determines that your intercompany pricing was not at arm's length â attracts tax on the adjustment plus interest (12% per annum) plus penalty (up to 300% of the tax on adjustment in some cases). Advance Pricing Agreements (APAs) are available for companies wanting certainty â we have experience in both unilateral and bilateral APAs.</SEOProseP>
      ...
      <SEOSection label="Cost & Fees" title="What transfer pricing compliance costs in India">
        <div style={{ background: T.stone, borderRadius: 14, padding: "28px 30px", border: `1px solid ${T.bdr}`, marginBottom: 16 }}>
          {[
            { item: "TP study + Form 3CEB (simple)", range: "â¹40,000 â â¹75,000", note: "Single transaction type, TNMM methodology" },
            { item: "TP study (multi-transaction)", range: "â¹75,000 â â¹1,50,000", note: "Multiple transaction types, complex benchmarking" },
            { item: "Annual TP retainer", range: "â¹60,000 â â¹1,20,000/year", note: "Study + 3CEB + ongoing monitoring" },
            { item: "TP audit representation", range: "â¹1,00,000 â â¹3,00,000", note: "Responding to scrutiny notices, DRP filings" },
            { item: "APA application", range: "â¹3,00,000 â â¹8,00,000", note: "Unilateral APA â 5-year pricing certainty" },
          ].map(r => (
            <div key={r.item} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 16, padding: "12px 0", borderBottom: `1px solid ${T.bdr}`, alignItems: "start" }}>
              <div>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: T.ch }}>{r.item}</div>
      ...
      <SEOSection label="FAQ" title="Transfer pricing questions">
        <SEOFAQs items={[
          { q: "At what transaction value is transfer pricing documentation required?", a: "Transfer pricing documentation is required when the aggregate value of international transactions with related parties exceeds â¹1 crore in a financial year. Form 3CEB is additionally required. Below this threshold, documentation is still best practice but not legally mandated." },
          { q: "What is the penalty for non-compliance with transfer pricing rules?", a: "Penalties range from 2% of the transaction value (for failure to maintain documentation) to 100â300% of the tax on any adjustment. Form 3CEB non-filing attracts â¹1 lakh flat penalty. In practice, the larger risk is the adjustment itself â which then attracts tax + interest + penalty." },
          { q: "How is transfer pricing different for a GCC?", a: "A GCC (Global Capability Centre) providing services to its foreign parent is subject to TP rules on those services. The standard approach is a cost-plus methodology â the GCC charges the parent its total costs plus a mark-up (typically 8â15%). This mark-up and the methodology must be benchmarked and documented annually." },
          { q: "What is an Advance Pricing Agreement (APA)?", a: "An APA is a formal agreement between a taxpayer and the CBDT fixing the transfer pricing methodology and pricing for a period of 5 years. It provides complete certainty â no TP audit risk during the APA period. Available as unilateral (India only) or bilateral (India + treaty partner country)." },
...
// âââ SEO PAGE 4: FDI RULES INDIA âââââââââââââââââââââââââââââââââââââââââââââ
